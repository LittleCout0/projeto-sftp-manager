    ...
    try:
        if MW_VERSION.name == 'v5.2.8':
            ...
            new_local_path = path_manager(
                localFolder.createLocalFolders(local_build_folder, STB_MODEL))  # STB Model folder
            ...
            for folder in path_product_folder:
                createLocalSubDirectories(sftp, path_manager.joinpath(build_path, path_manager(folder)).as_posix(),
                                          path_manager.joinpath(new_local_path, folder), MW_VERSION, is_folder_zip)
            ...
            if not is_folder_zip:
                ...
                for remote_dir in remote_path:
                    ...
                    new_local_path = path_manager(
                        localFolder.createLocalFolders(folders_dict.get(''.join(model)), remote_dir.name))
            ...
            if not (''.join(model) in sub_folders_513_dict):
                ...
                if path_manager.joinpath(local_path, model[0]).exists():
                    sub_folders_513_dict[''.join(model)] = path_manager(
                        localFolder.createLocalFolders(path_manager.joinpath(local_path, model[0]), model[1]))
                
                else:
                    sub_folders_513_dict[''.join(model)] = path_manager(
                        localFolder.createLocalFolders(localFolder.createLocalFolders(local_path, model[0]), model[1]))
...
def downloadFiles(sftp, current_path, remote_path, MW_VERSION, is_folder_zip):
    ...
    callback_progressbar, progressbar = progressBarView(ascii=False, desc=file_name_for_progress_bar, unit='b',
                                                        unit_scale=True)
        ...
        for remote_dir in remote_path:
            ...
            for remote_file in sftp.listdir_attr(remote_dir.as_posix()):
                ...
                if remote_file.filename not in FILES_TO_NOT_DOWNLOAD and not is_carousel[0] == 'CAROUSEL':
                    ...
                    sftp.get(path_manager.joinpath(remote_dir, remote_file.filename).as_posix(),
                             path_manager.joinpath(current_path[i], remote_file.filename), callback_progressbar)
            log.info(f'Files downloaded:{path_manager.joinpath(current_path[i], remote_file.filename)}')
            i += 1
        progressbar.close()
    
    # MW 5.2.4 could have zip folder 
    elif MW_VERSION.name == 'v5.2.4' and is_folder_zip:
        for remote_file in sftp.listdir_attr(remote_path.as_posix()):
            if remote_file.filename not in FILES_TO_NOT_DOWNLOAD:
                print(remote_file.filename)
                sftp.get(path_manager.joinpath(remote_path, remote_file.filename).as_posix(),
                         path_manager.joinpath(current_path, remote_file.filename), callback_progressbar)
    ...
    elif MW_VERSION.name == 'v5.2.4' and not is_folder_zip:
        i = 0
        for remote_dir in remote_path:
            print(remote_dir)
            for remote_file in sftp.listdir_attr(remote_dir.as_posix()):
...
def startDownloadProcess(PATH_ROOT, MW_VERSION, STB_MODEL, NET, client, latestBuild_path, local_build_folder):
    PATH_MW = path_manager.joinpath(PATH_ROOT, MW_VERSION, NET,
                                    latestBuild_path)  # /nfs/OpentvOS/v5.x.x/NET/Build6.x.x.x
