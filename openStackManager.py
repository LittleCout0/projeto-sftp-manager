                    ...
                    if(HUMAX_MODEL in url.split('/')):
                        if('2t' in url.split('_')):
                            try:
                                ...
                                log.info(f'Downloading build: {url}')
                                download_url(url, path_manager.joinpath(sub_folders_513_dict['HUMAX_2T'], url.split('/')[-1]))                                                   
                            except urllib.error.ContentTooShortError:
                                pass
                        ...
                        if('2t' in url.split('_')):
                            try:
                                print(f'Tech 2T:{url}')
                                log.info(f'Downloading build: {url}')
                                download_url(url, path_manager.joinpath(sub_folders_513_dict['TECH_2T'], url.split('/')[-1]))
                            except urllib.error.ContentTooShortError:
                                pass
