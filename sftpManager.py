    ...
    if client:
        for MW in MW_VERSION_LIST:
            if MW == MW_VERSION_528:
                latestBuild_path = getLatestBuildNameFromServer(client, path_manager.joinpath(ROOT, MW_VERSION_528,
                                                                                              NET))  # Receive the build path: Build6.x.x.x
                latestBuild_clean = getBuildNameFromDir(
                    latestBuild_path)  # Remove "Build" from name to create folder locally: 6.x.x.x
                ...
                latestBuild_path = getLatestBuildNameFromServer(client, path_manager.joinpath(ROOT, MW_VERSION_524,
                                                                                              NET))  # Receive the build path: Build4.x.x.x
                latestBuild_clean = getBuildNameFromDir(
                    latestBuild_path)  # Remove "Build" from name to create folder locally: 4.x.x.x
                ...
                latestBuild_path = getLatestBuildNameFromServer(client, path_manager.joinpath(ROOT, MW_VERSION_513,
                                                                                              NET))  # Receive the build path: Build2.x.x.x
                latestBuild_clean = getBuildNameFromDir(
                    latestBuild_path)  # Remove "Build" from name to create folder locally: 2.x.x.x
